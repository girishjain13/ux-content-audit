...
import { isLikelyNonHtmlResource, normalizeCrawlUrl, urlHost } from "./url";
...
function buildExtractionScript(rootHost: string): string {
  return `(() => {
      function abs(href) {
        try {
...
async function settleJavascript(page: PlaywrightPage): Promise<number> {
  const started = Date.now();
  try {
    await page.waitForLoadState("networkidle", { timeout: 4500 });
  } catch {
    /* trackers often never idle — that's expected on JS-heavy sites */
  }
  try {
    await page.evaluate(async () => {
      window.scrollTo(0, Math.min(document.body.scrollHeight * 0.45, 1400));
  ...
  try {
    await page.waitForFunction(
      () => (document.body?.innerText || "").trim().split(/\s+/).length > 40,
      { timeout: 2500 },
    );
  } catch {
    /* SPA never grew — still capture what we have */
  }
...
async function renderOnePage(
  ...
  try {
    let response;
    try {
      response = await page.goto(url, { waitUntil: "domcontentloaded", timeout: 28000 });
    } catch {
      response = await page.goto(url, { waitUntil: "commit", timeout: 18000 });
    }
    await page.waitForTimeout(900);

    const wordCountBeforeConsent = await page.evaluate(() =>
    ...
    const extracted = (await page.evaluate(buildExtractionScript(rootHost))) as Record<string, unknown>;
    ...
    const signals: PageSignals = {
      htmlLang: (extracted.htmlLang as string | null) ?? null,
      htmlDir: (extracted.htmlDir as string | null) ?? null,
      cookieBannerVisible: Boolean(extracted.cookieBannerVisible),
      cookieGatingCopy: Boolean(extracted.cookieGatingCopy),
      cookieDismissed,
      iframeSrcs: (extracted.iframeSrcs as string[]) ?? [],
      iframeGatedCount: Number(extracted.iframeGatedCount ?? 0),
      duplicateNavLabels: (extracted.duplicateNavLabels as string[]) ?? [],
      cmsLeftovers: (extracted.cmsLeftovers as string[]) ?? [],
      chatWidget: Boolean(extracted.chatWidget),
      searchPlaceholder: (extracted.searchPlaceholder as string | null) ?? null,
      emptyAltCount: Number(extracted.emptyAltCount ?? 0),
      imageCount: Number(extracted.imageCount ?? 0),
      navLabels: (extracted.navLabels as string[]) ?? [],
      hydrationWaitedMs,
    ...
    return {
      ...
      depth: 0,
      title: (extracted.title as string | null) ?? null,
      metaDescription: (extracted.metaDescription as string | null) ?? null,
      h1Text: (extracted.h1Text as string | null) ?? null,
      canonical: (extracted.canonical as string | null) ?? null,
      wordCount: Number(extracted.wordCount ?? 0),
      htmlLang: (extracted.htmlLang as string | null) ?? null,
      isClientRendered: Boolean(extracted.isClientRendered),
      internalLinks: (extracted.internalLinks as string[]) ?? [],
      externalLinks: (extracted.externalLinks as string[]) ?? [],
      accessibilityViolations,
      detectedGlobals: (extracted.detectedGlobals as string[]) ?? [],
      nonFunctionalHrefs: (extracted.nonFunctionalHrefs as string[]) ?? [],
      error: null,
      signals,
...
async function loadRobots(startUrl: string): Promise<string> {
  try {
    const res = await fetch(new URL("/robots.txt", startUrl).toString(), {
...
function robotsAllows(robotsTxt: string, url: string): boolean {
...
async function discoverSitemapUrls(startUrl: string, cap: number): Promise<string[]> {
  try {
    const res = await fetch(new URL("/sitemap.xml", startUrl).toString(), {
    ...
    return locs
      .filter((u) => urlHost(u) === host && !isLikelyNonHtmlResource(u))
      .slice(0, cap);
  } catch {
    return [];
  }
    ...
    while (queue.length > 0 && results.length < maxPages) {
      ...
      const batchResults = await Promise.all(
        batch.map(async ({ url, depth }) => {
          if (respectRobots && !robotsAllows(robotsTxt, url)) {
            return {
              url,
              finalUrl: url,
              statusCode: null,
      ...
      for (const result of batchResults) {
        ...
        if (result.depth < maxDepth) {
          for (const link of result.internalLinks) {
            ...
            seen.add(n);
            queue.push({ url: n, depth: result.depth + 1 });
          }
        }
      }
    }
